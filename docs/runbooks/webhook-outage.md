...
2. **Check the endpoint is reachable** — `curl -X POST https://forge.solana.tools/api/webhooks/helius -H "Content-Type: application/json" -d '{}'`. You should get a 401 (unauthorized) or 400 (bad payload) — a 5xx means the endpoint itself is broken.
