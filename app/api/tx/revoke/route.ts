...
import { revokeSchema } from "@/lib/solana/validate";
import { buildRevokeTx } from "@/lib/solana/buildRevoke";
...
export async function POST(req: Request) {
  try {
    ...
    const tx = await buildRevokeTx(body.data);
    ...
    const status = message.includes("not the") ? 403 : 500;
