      ...
      const { signature } = await execute(
        () =>
          fetch("/api/tx/revoke", {
            ...
            body: JSON.stringify({ payer: publicKey.toBase58(), mint, type: "freeze" }),
          }).then(async (r) => {
          ...
          }),
        {
          ...
          onConfirmed: async (s) => {
            await fetch("/api/confirm", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ signature: s, action: "revokeFreeze", wallet: publicKey.toBase58(), mint }),
            });
          },
        }
      );
          ...
          <>
            ...
            <div className="wizard-actions">
              {txState !== "idle" && txState !== "failed" ? (
                ...
                >
                  {publicKey ? "Revoke Freeze Authority" : "Connect Wallet"}
                </button>
              )}
            </div>
          </>
