...
export default function MakeImmutablePage() {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      if (!info.isMutable) {
        setCheckError("This token is already immutable.");
        setPhase("input");
  ...
  async function handleRevoke() {
    ...
    try {
      const { signature } = await execute(
        () =>
          fetch("/api/tx/revoke", {
            ...
            body: JSON.stringify({ payer: publicKey.toBase58(), mint: mint.trim(), type: "update" }),
          }).then(async (r) => {
          ...
          }),
        {
          ...
          onConfirmed: async (s) => {
            await fetch("/api/confirm", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ signature: s, action: "makeImmutable", wallet: publicKey.toBase58(), mint: mint.trim() }),
            });
          },
        }
      );
      ...
      setTxError(parseError(e, "make-immutable"));
      setTxState("failed");
      setPhase("confirmed");
    }
  }
