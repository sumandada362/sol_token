...
export default function RevokeMintPage() {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      if (!info.mintAuthority) {
        setCheckError("Mint authority is already revoked for this token.");
        setPhase("input");
  ...
  async function handleRevoke() {
    ...
    try {
      const { signature } = await execute(
        () =>
          fetch("/api/tx/revoke", {
            ...
            body: JSON.stringify({ payer: publicKey.toBase58(), mint: mint.trim(), type: "mint" }),
          }).then(async (r) => {
          ...
          }),
        {
          ...
          onConfirmed: async (s) => {
            await fetch("/api/confirm", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ signature: s, action: "revokeMint", wallet: publicKey.toBase58(), mint: mint.trim() }),
            });
          },
        }
      );
      ...
      setTxError(parseError(e, "revoke-mint"));
      setTxState("failed");
      setPhase("confirmed");
    }
  }
            ...
            <div className="tx-failed" style={{ textAlign: "center", padding: "1.5rem 0" }}>
              ...
              {tokenInfo && (
                <p className="lp-mono" style={{ fontSize: "0.78rem", color: "var(--lp-muted)", marginBottom: "1rem" }}>
                  Authority: {tokenInfo.mintAuthority?.slice(0, 8)}…{tokenInfo.mintAuthority?.slice(-8)}
                </p>
              )}
