...
export default function FreezeAccountPage() {
  ...
  async function handleFreeze() {
    ...
    try {
      const { signature } = await execute(
        () =>
          fetch("/api/tx/freeze-accounts", {
            ...
            body: JSON.stringify({ payer: publicKey.toBase58(), mint: mint.trim(), wallets: validWallets, type: "freeze" }),
          }).then(async (r) => {
          ...
          }),
        {
          ...
          onConfirmed: async (s) => {
            await fetch("/api/confirm", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ signature: s, action: "freezeAccounts", wallet: publicKey.toBase58(), mint: mint.trim(), count: validWallets.length }),
            });
          },
        }
      );
      ...
      setTxError(parseError(e, "freeze-account"));
      setTxState("failed");
      setPhase("form");
    }
  }
      ...
      <div className="page-wrap page-wrap--narrow">
        <div className="tool-header">
          ...
          <p className="page-sub">
            Freeze token accounts — blocks all transfers for those wallets. Requires freeze authority.
          </p>
        ...
        {phase === "done" && (
          <div className="tx-success lp-card">
            ...
            <p className="tx-success-label">{validWallets.length} account{validWallets.length !== 1 ? "s" : ""} frozen</p>
            <p className="burn-remaining">Those wallets can no longer transfer {tokenInfo?.symbol}.</p>
          ...
          <>
            <div className="lp-card burn-card">
              ...
              <div className="burn-warning" style={{ marginTop: "1rem" }}>
                ...
                <div>Frozen accounts <strong>cannot send or receive</strong> {tokenInfo?.symbol} until unfrozen.</div>
              </div>
            ...
            <div className="wizard-actions">
              {phase === "signing" && txState !== "failed" ? (
                ...
                <>
                  ...
                  <button className="lp-btn lp-btn--primary burn-btn" disabled={validWallets.length === 0} onClick={handleFreeze}>
                    Freeze {validWallets.length > 0 ? `${validWallets.length} Account${validWallets.length !== 1 ? "s" : ""}` : "Accounts"}
                  </button>
                </>
              )}
            </div>
